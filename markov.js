...
class Markov {
  constructor(readCompiled = false){
    /*
  ...
  writeCompiled() {
    return new Promise((resolve, reject) => {
      fs.writeFile(COMPILED, JSON.stringify(this.genMap), "utf8", (error) => {
        if (error)
          return reject(error)
        console.log("Saved compiled genMap")
        resolve()
      })
    })
  }
  ...
  */
  generateSentences(count, seed) {
    ...
    /* TODO: prevent the first word of the sentence from being punctuation
    ...
    */
    ...
    while (!seed || !(seed in this.genMap)) {
